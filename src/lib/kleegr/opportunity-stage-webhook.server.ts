/**
 * Shared handler for the public opportunity-stage webhook, mounted at TWO
 * URLs by thin route wrappers:
 *
 *   /api/public/webhooks/crm/opportunity-stage   <- canonical (whitelabel-clean)
 *   /api/public/webhooks/ghl/opportunity-stage   <- legacy alias; existing CRM
 *                                                   workflows still point here,
 *                                                   never remove without
 *                                                   repointing them first
 *
 * Auth: HMAC `x-kleegr-signature: sha256=<hex>` OR shared header
 * `x-kleegr-secret: <secret>`.
 *
 * Payload (CRM "Custom Webhook" action) — FULLY AUTOMATED shape:
 *   {
 *     event_id?: string,
 ...
 *     unit_crm_id?: string,            // optional override — normally auto-fetched
 *     building_crm_id?: string,        // optional — for whole-building sales
 *   }
 *
 * The backend automatically calls the CRM API to find the Unit/Building
 ...
 * obeyed (verifyUnitHint).
 *
 * DEDUPE — read this before changing it:
 * CRM workflows typically build event_id as "{{opportunity.id}}-{{stage}}", so
 ...
 * What a duplicate actually means here is a RETRY: the CRM re-delivers a
 * failed webhook within a couple of minutes. Hence a short time window,
 * rather than "seen before, ever".
 */
import { createHmac, timingSafeEqual } from "crypto";

/** A repeat inside this window is a CRM retry; outside it, a genuine move. */
const DEDUPE_WINDOW_MS = 2 * 60 * 1000;

export async function handleOpportunityStagePost(request: Request): Promise<Response> {
  ...
  if (outcome.outcome === "no_unit_reference") {
    ...
    return json({
      ...
      pending: true,
      message:
        "No unit associated yet. Event queued — will apply automatically once a unit is added to this opportunity as Locked/Reserved.",
    });
  }
...
export function webhookOptionsResponse(): Response {
